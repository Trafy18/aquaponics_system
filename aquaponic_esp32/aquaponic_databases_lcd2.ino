...
const unsigned long interval = 5000; // Interval waktu (2 detik)
int displayState = 0; // Menentukan tampilan aktif
...
void sendData(){
  ...
  delay(1000);
   ...
   // ph
    ...
    volume = pi * pow(r,2)* ketinggianAir;
    ...
    // Periksa apakah waktu interval sudah tercapai
    if (currentMillis - previousMillis >= interval) {
        previousMillis = currentMillis; // Reset waktu
        displayState = (displayState + 1) % 4; // Ganti tampilan (0, 1, 2)
    }

    // Bersihkan layar
    display.clearDisplay();

    // Tentukan tampilan berdasarkan nilai displayState
    if (displayState == 0) {
        // Tampilan suhu
  ...
  String sensorData4 = String(volume);
  ...
  http.begin( wifiClient,"http://akuaponikunikom.my.id//postData.php");
